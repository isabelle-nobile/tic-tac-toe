...
class Minimax:
        ...
        for i in range(3):
            for j in range(3):
                if board[i][j] == ' ':
                    ...
                    score = self.minimax(board_copy, 2, False, current_player, ai_player)
    ...
    def check_win(self, board, player):
        """
        Cette fonction vérifie si le joueur donné a gagné la partie.
        """
        ...
        # Si aucune des conditions précédentes n'a été vérifiée, le joueur n'a pas gagné
        return False


    def check_tie(self, board, player):
        """
        Cette fonction vérifie s'il y a une égalité (match nul) dans le jeu.
        """
        ...
        # Si toutes les cases sont remplies et qu'aucun joueur n'a gagné, il y a une égalité
        return not self.check_win(board, player) and not self.check_win(board, 'X' if player == 'O' else 'O')
